...
if (!empty($_FILES["nama_file"]["tmp_name"]))
{
	...
	{			
		...
		if (move_uploaded_file($_FILES['nama_file']['tmp_name'], $gambar)) {
			...
			$sql2="UPDATE custom SET status='Menuggu Konfirmasi Oleh admin' WHERE kode='$nopo'" or die(mysqli_error());
			$res2=mysqli_query($koneksi, $sql2) or die (mysqli_error());
			//echo "Gambar berhasil dikirim ke direktori".$gambar;
            echo "<script>alert('Data Konfirmasi berhasil diupdate!'); window.location = 'index1.php?link=cs'</script>";	   
		} else {
            echo "<script>alert('Terjadi Kesalahan!'); window.location = 'index1.php?link=cs'</script>";	   
